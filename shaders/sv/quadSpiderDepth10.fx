...
///////////////////////////////////////////////////////////////////////////////
//
//// float4x4 gWVP : WorldViewProjection; // already defined by common
//
...
psOutput testPS(vsOutput input)
{
	...
	float radius = 0.5;
	float softness = 0.1;
	float2 dist = input.uv - float2(0.5, 0.5);
	float4 color = 1.0 - smoothstep(radius - (radius * softness), radius + (radius * softness), dot(dist, dist) * 4.0);
